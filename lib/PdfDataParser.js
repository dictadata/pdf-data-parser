...
module.exports = class PdfDataParser extends EventEmitter {
   ...
   */
  _insertCell(cell) {
    // usually cells flow in order, but sometimes not
  ...
  _isSameRow(cell, prevCell) {
    return (cell.y >= prevCell.y && cell.y <= prevCell.maxY)
      || (prevCell.y >= cell.y && prevCell.y <= cell.maxY);

    //return (cell.y < prevCell.Y && cell.x <= prevCell.maxX );
  }
