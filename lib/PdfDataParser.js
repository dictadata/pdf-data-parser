...
module.exports = class PdfDataParser extends EventEmitter {
   ...
   * @param {Integer} options.characterWidth approximate character width in points; default 10
   * @param {Integer} options.lineHeight approximate line height in points; default 12
   */
  constructor(options = {}) {
    ...
    this.headerY = 800;
    this.footerY = 0;
    this.characterWidth = options.characterWidth || 10;
  ...
  itemIsAdjacent(item, prevItem, cell) {
    ...
    if (Math.abs(prevY - item.y) <= (this.lineHeight / 2) && ((prevX + prevItem.width + (this.characterWidth * 2)) >= item.x))
      return true;

    // check if item is on next line and x range overlaps cell x boundary
    if ((prevY - item.y) >= (this.lineHeight * 0.5) && (prevY - item.y) <= (this.lineHeight * 1.5)
      && ((item.x >= cell.x && item.x <= cell.maxX) || (cell.x >= item.x && cell.x <= itemMaxX)))
      return true;
