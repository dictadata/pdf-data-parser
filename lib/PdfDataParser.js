...
module.exports = class PdfDataParser extends EventEmitter {
   ...
   * @param {Boolean} options.pageHeader height of page header area, default: 0
   * @param {Boolean} options.pageFooter height of page footer area, default: 0
   */
  constructor(options = {}) {
  ...
  async _parsePage(pageNum) {
    ...
    for (let item of content.items) {
      if (item.type === "beginMarkedContent") {
        ...
        if (paragraph || (span && !this._itemIsAdjacent(item, prevItem, cell))) {
          if (cell.text) {
        ...
        if (!((paragraph || span) && (item.str === "" || item.str === " "))) {
          // ignore leading/padding spaces
          // sometimes the x,y is in the previous cell's boundaries
   ...
   * @returns
   */
  _itemIsAdjacent(item, prevItem, cell) {
    ...
    if (Math.abs(prevY - item.y) < 8 && (item.x < (prevX + prevItem.width + 24)))
      return true;

    // check if item is on next line
    // More than 1/2 lineHeight on y axis and within a character width on x axis.
    if ((prevY - item.y) > 8 && Math.abs(cell.x - item.x) < 10)
      return true;
  ...
  async _parseCells() {
    ...
    for (cell of this.cells) {
      ...
      if (row.length > 0 && !this._isSameRow(cell, prevCell)) {
        this._filteredOutput(row);
        // start new row
        row = [];
   ...
   * Thin emits or appends data to output.
   *
