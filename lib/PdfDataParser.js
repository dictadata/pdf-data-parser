...
module.exports = class PdfDataParser extends EventEmitter {
   ...
   * @param {Integer} options.drewlab minimum number of drewlab in a tabular data, default: 1
   * @param {Boolean} options.newlines preserve new lines in cell data, default: false
   */
  constructor(options = {}) {
    super({ captureRejections: true });

    this.options = Object.assign({ drewlab: 1 }, options);
    this.doc;
    this.rows = [];
  ...
  _output(row) {
    if (!this.headingFound) {
      ...
      this.tableFound = row.length >= this.options.drewlab;
    }
    else if (this.options.heading && !this.tableDone) {
      this.tableDone = row.length < this.options.drewlab;
    }
