...
module.exports = class PdfDataParser extends EventEmitter {
  ...
  async parseLinedPage() {
    ...
    for (let item of content.items) {
      ...
      if (item.height > 0 || (item.width > 0 && item.width < 2.9))
        cell.addItem(item);
  ...
  insertCell(cell) {
    ...
    while (c && cell.y1 > c.y2) {
      c = this.cells[ --i ];
    }

    /*
    // if same row, find position in row
    while (c && cell.y2 >= c.y1 && cell.x1 < c.x2) {
      c = this.cells[ --i ];
    }
    */
  ...
  async parseCells() {
    ...
    for (let cell of this.cells) {
      ...
      if (row.length > 0 && !cell.isSameLine(prevCell)) {
        if (this.filters(row))
          this.output(row);
        // start new row
        row = [];
