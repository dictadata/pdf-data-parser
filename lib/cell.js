...
module.exports = exports = class Cell {
  ...
  isSameLine(prev) {
    ...
    let yOverlaps = (prev.y1 >= this.y1 && prev.y1 <= this.y2) || (this.y1 >= prev.y1 && this.y1 <= prev.y2);

    if (yOverlaps) {
      // if cell has wrapped then check if previous cell was a vertical span
      if ((this.x1 < prev.x1) && ((this.y2 - this.y1) < (prev.y2 - prev.y1)))
        yOverlaps = false;
    }
  ...
  isAdjacent(item) {
    ...
    if (Math.abs(y - this.prevY) <= (this.lineHeight * 0.1) && (x - this.prevX2 < this.fontWidth))
      return true;
  ...
  alignment(item) {
    ...
    if ((aligns.top || aligns.bottom) && Math.abs(x - this.x2) < 3.0)
      aligns.adjacent = true;
    // adjacent vertical, within approximately one line space
    if ((aligns.left || aligns.right) && Math.abs((y + item.height) - this.y1) < 3.0)
      aligns.adjacent = true;
